...
\subsection{Buffer dissymétrique}
La création d'un buffer dissymétrique est nécessaire pour cartographier les zones exploitables par skidder en montagne.

<<eval=F>>=
projet <- st_read("~/pCloud Sync/Cours/APT3/MicroProjetsR/2018/Micro1/Projet.shp", quiet=T)
mnt <- SiteMnt(projet, buffer=400, zoom=14)
# mnt <- raster("/Users/maxbruciamacchie/Desktop/Pimodan/raster/MNT/MNT.tif")

buf <- BufferDissy(projet, mnt, conc=1.8)
