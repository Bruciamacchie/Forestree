...
\subsubsection{EvolEssence}
La fonction \textbf{\texttt{EvolEssence}} utilise les bases de données de l'IFN pour analyser la répartion d'une essence par classe de diamètre et découpage de la France métropolitaine en greco ou type de climat.
<<eval=F>>=
...
La figure \ref{CompareMNT} peut être complétée par la figure \ref{CompareAWSIGN} qui utilise les points du réseau géodésiques de l'IGN. Dans ette dernière les altitudes en abscisse sont celles du réseau géodésique. Elles servent de référence. Les altitudes en ordonnée sont celles extraites des 2 sources, Aws et IGN. Le réseau géodésique de l'IGN permet de distinguer les points de type 1 plus précis que ceux de type 2. La figure \ref{CompareMNT} permet de constater que les altitudes extraites du MNT de l'IGN ne correspondent pas parfaitement à celles mesurées sur les points géodésiques.

\begin{figure}[H]
\begin{center}
 \includegraphics[width=14cm]{/Users/maxbruciamacchie/pCloud Sync/Packages/Forestree/vignettes/Images/CompareAWSIGN.png}
\caption{\emph{Comparaison des altitudes extraites des MNT issus du serveur Aws ou IGN avec celles mesurées sur les points géodésiques.}}
\label{CompareAWSIGN}
\end{center}
\end{figure}
...
\subsubsection{Harmonisation rasters}\label{ParaHarmo}
Lorsque l'on souhaite utiliser ces données (MNS, MNT) (voir paragraphe \ref{ParaMNH}) qui se présentent généralement sous forme de rasters, la première difficulté à surmonter est de les harmoniser car en général elles n'ont pas même résolution, point d'origine ou zone d'extension. C'est l'objectif de la fonction \texttt{HarmoniseRaster}.
<<eval=F>>=
...
\subsubsection{CreateStack}
La fonction \texttt{\textbf{CreateStack}} permet également d'harmoniser plus de 2 rasters à partir d'une liste, le premier servant également de référence. cette fonction permet de découper l'ensemble des rasters selon un périmètre, mais aussi d'exclure selon un second polygone.

<<eval=F>>=
mns <- system.file("MNS.tif", package = "Forestree")
mnt <- system.file("MNT.tif", package = "Forestree")
irc <- system.file("IRC.tif", package = "Forestree")
listeRaster <- c(mns,mnt,irc)
data(cad)

s <- CreateStack(listeRaster, noms=c("mns","mnt","irc"), cad)
plot(s)
@
...
La fonction Mosaic assemble tous les rasters au format TIF ou ASC contenus dans un dossier en un seul fichier au format TIF. Attention si les différents rasters se supperposent il erst préférable d'utiliser la fonction merge.
<<eval=F>>=
Mosaic()
@
...
\subsubsection{Calcul du MNH}\label{ParaMNH}

\begin{minipage}{0.45\linewidth}
Les MNS et MNT harmonisés grâce à la fonction \texttt{\textbf{HarmoniseRaster}} (voir paragraphe \ref{ParaHarmo}) permettent par simple différence de calculer des Modèles Numériques de Hauteur (MNH). Dans la figure \ref{MNH} l'échelle à droite est celle des hauteurs.
<<eval=F>>=
mns <- result[[1]]
mnt <- result[[2]]
mnh <- mns-mnt
...
\section{Desserte}
\subsection{Profil en long}
La fonction \texttt{\textbf{DessertePL}} permet de visualiser le profil en long de tout élément de la desserte.
<<eval=F>>=
projet <- st_read("~/pCloud Sync/Cours/APT3/MicroProjetsR/2018/Micro1/Projet.shp", quiet=T)
mnt <- SiteMnt(projet, buffer=400, zoom=12)
DessertePL(projet[2,], mnt, pas=50)
@

\subsection{Cartographie des zones exploitables par tracteur}

\subsubsection{fonction AmontAval}
La création d'un buffer dissymétrique est nécessaire pour cartographier les zones exploitables par skidder en montagne. C'est l'objet de la fonction \texttt{\textbf{AmontAval}}. Elle s'appuie sur la desserte existante (routes, pistes). Elle possède comme argument un buffer amont (par défaut 50m), un buffer aval (par défaut 150m), le choix d'une résolution le long de la desserte, ainsi qu'un indice de concavité. La fonction \texttt{\textbf{AmontAval}} peut être utilisée seule, mais ne peut être appliquée qu'à un seul linéaire. C'est le cas de la deuxième portion de route dans l'exemple ci-après.

<<eval=F>>=
projet <- st_read("~/pCloud Sync/Cours/APT3/MicroProjetsR/2018/Micro1/Projet.shp", quiet=T)
mnt <- SiteMnt(projet, buffer=400, zoom=14)

num=2
buf <- AmontAval(projet[num,], mnt, amont=50, aval=150, pas=50, conc=1.8)

plot(mnt, axes=F, box=F)
plot(st_geometry(projet[num,]), add=T)
plot(st_geometry(buf), add=T, border='red')
@

\begin{figure}[H]
\begin{center}
 \includegraphics[width=10cm]{/Users/maxbruciamacchie/pCloud Sync/Packages/Forestree/Vignettes/Images/AmontAval.png}
\caption{\emph{Exemple de buffer dissymétrique le long d'une desserte.}}
\label{AmontAval}
\end{center}
\end{figure}

\subsubsection{fonction BufferDissy}
La fonction \texttt{\textbf{BufferDissy}} permet de calculer le buffer dissymétrique le long de plusieurs éléments de desserte contigus.
<<eval=F>>=
# projet <- st_read("~/pCloud Sync/Cours/APT3/MicroProjetsR/2018/Micro1/Projet.shp", quiet=T)
# mnt <- SiteMnt(projet, buffer=400, zoom=14)

buf <- BufferDissy(projet, mnt, amont=50, aval=150, pas=50, conc=1.8)

plot(mnt, axes=F, box=F)
plot(st_geometry(projet), add=T)
plot(st_geometry(buf), add=T, border='red')
@


\subsubsection{Zones exploitables par tracteurs}
La fonction \texttt{\textbf{ExploiTracteur}} complète la fonction \texttt{\textbf{BufferDissy}} en incluant les zones planes. Dans la figure \ref{ExploiTracteur}, les zones grisées sont considérées comme planes, la desserte est en bleu, les zones planes accessibles par la route sont en vert, la zone exploitable par tracteur est délimitée en rouge.
<<eval=F>>=
# projet <- st_read("~/pCloud Sync/Cours/APT3/MicroProjetsR/2018/Micro1/Projet.shp", quiet=T)
# mnt <- SiteMnt(projet, buffer=400, zoom=14)

buf <- ExploiTracteur(projet, mnt, amont=50, aval=150, pas=50, conc=1.8, plane=0.3)
@

\begin{figure}[H]
\begin{center}
 \includegraphics[width=10cm]{/Users/maxbruciamacchie/pCloud Sync/Packages/Forestree/Vignettes/Images/ExploiTracteur.pdf}
\caption{\emph{Cartographie de la surface exploitable par tracteur.}}
\label{ExploiTracteur}
\end{center}
\end{figure}

<<eval=F>>=
projet <- st_read("~/pCloud Sync/Cours/APT3/MicroProjetsR/2018/Micro1/Projet.shp", quiet=T)
mnt <- SiteMnt(projet, buffer=400, zoom=13)
