...
\vspace{2cm}
Ce package R a été construit dans le cadre de la formation des ingénieurs forestiers (FIF) d'AgroParisTech. Il contient des fonctions permettant de faciliter le travail des gestionnaires forestiers dans des domaines variés tels que la dendrométrie, la gestion foncière, la cartographie, l'évaluation économique, l'estimation forestière ou de la gestion des risques.\\
...
print(xtable(res$tab,
      ...
      digits=c(0,0,rep(1,4),rep(2,4)),label = "FindSch", caption.placement = "top"),
...
\subsubsection{Constitution base IFN}

La fonction \texttt{IFNdata} permet de télécharger les données brutes de l'IFN à partir du site
\begin{center}
http://inventaire-forestier.ign.fr/spip/IMG/zip/
\end{center}
et de les enregistrer sous forme d'une archive Rdata contenant deux tables, IFNarbres et IFNplacettes. La ligne de code ci-après permet de créer une archive entre les années 2011 et 2016.
<<eval=F>>=
IFNdata(20011,2016)
@
...
La fonction \texttt{IFNacctD} renvoie les accroissements sur le diamètre par essence et classe de diamètre en utilisant la base de données arbres de l'IFN. La fonction nécessite en entrée un shape correspondant au périmètre retenu : forêt, massif, sylvoécorégion, etc. La fonction renvoie un tableau des accroissements sur le diamètre par essence et classe de diamètre. Elle intègre les coefficients d'écorce, les accroissements sont donc sur écorce.\\

La fonction \texttt{IFNacctD} possède les arguments suivants : \\
- file : nom du fichier au format shp. Si non renseigné ouverture d'une boîte de dialogue. \\
- TailleBuffer : Taille du buffer en mètre. Par défaut elle est égale à 3000 m. \\
SeuilCircf : seuil minimal de circonférence en cm en dessous duquel l'arbre n'est pas retenu pour le calcul du tarif. Par défaut ce seuil est fixé à 50 cm. \\
- SeuilNb : seuil minimal du nombre d'arbres dans la base IFN pour qu'une essence soit retenue. Par défaut ce seuil est fixé à 10 tiges. \\
- res : résolution (par défaut = 1000 m) \\
- UseSer : argument permettant de choisir si le calcul des numéros de tarif doit se faire au sein d'une même sylvoécorégion. Par défaut, UseSer=TRUE. \\
- enreg : argument permettant de choisir l'enregistrement ou pas du tableau au format .xlsx. Par défaut enreg=FALSE.\\

\textbf{Attention} : cette fonction utilise les archives de l'IFN qui doivent être créées au préalable avec la fonction \texttt{IFNdata}.
<<eval=F>>=
data(perim)
load("IFNdata.Rdata")
res <- IFNacctD(file=perim, TailleBuffer=1000)
...
\begin{figure}[H]
\begin{center}
 \includegraphics[width=10cm]{/Users/maxbruciamacchie/Dropbox/Packages/Forestree/Vignette/Images/Protect.png}
...
La fonction \texttt{SiteMnt} est basée sur la fonction \texttt{get\_elev\_raster} du package \texttt{elevatr} qui va rechercher sur le serveur de l'Amazon Web Services le raster correspondant à l'extension du spatialPolygon fourni en entrée. Le paramètre zoom doit être un entier. Il permet de choisir la résolution du raster en sortie. Par défaut le paramètre zoom est égal à 14. Dans ce cas la résolution sera égale à environ 3,2 m. Si zoom est égal à 1 la résolution est de 7600 m, c'est pourquoi la fonction \texttt{SiteMnt} ramène automatiquement le zoom à 10 (résolution d'environ 51 m) si le zoom choisi par l'opérateur est inférieur à 10.\\

La figure \ref{CompareMNT} permet de comparer le MNT de l'IGN vendu à une résolution de 5 m avec celui issu du serveur d'Amazon fourni avec une résolution annoncée légèrement supérieure à 3 m. La forêt retenue pour cette comparaison est située sur la commune d'Allevard en Isère, avec des altitudes comprises entre 600 et 2500 m. Cette figure montre que la résolution réelle d'Amazon est supérieure à 5m. Son principal intérêt est qu'elle est gratuite, disponible sur l'ensemble de la planète et de résolution largement suffisante pour de nombreux emplois en forêt.

\begin{figure}[H]
\begin{center}
 \includegraphics[width=15cm]{/Users/maxbruciamacchie/Dropbox/Packages/Forestree/Vignette/Images/CompareMNT.png}
\caption{\emph{Comparaison des résolutions entre le MNT de l'IGN (à gauche} avec celui d'Amazon (à droite)}
\label{CompareMNT}
\end{center}
\end{figure}
...
\begin{figure}[H]
\begin{center}
 \includegraphics[width=15cm]{/Users/maxbruciamacchie/Dropbox/Packages/Forestree/Vignette/Images/ClassePente.pdf}
...
\begin{figure}[H]
\begin{center}
 \includegraphics[width=15cm]{/Users/maxbruciamacchie/Dropbox/Packages/Forestree/Vignette/Images/Courbes.pdf}
...
\begin{figure}[H]
\begin{center}
 \includegraphics[width=8cm]{/Users/maxbruciamacchie/Dropbox/Packages/Forestree/Vignette/Images/Cigal.png}
...
\begin{figure}[H]
\begin{center}
 \includegraphics[width=6cm]{/Users/maxbruciamacchie/Dropbox/Packages/Forestree/Vignette/Images/MNH17.png}
...
\begin{figure}[H]
\begin{center}
 \includegraphics[width=6cm]{/Users/maxbruciamacchie/Dropbox/Packages/Forestree/Vignette/Images/Houppier.png}
...
\begin{figure}[H]
\begin{center}
 \includegraphics[width=6cm]{/Users/maxbruciamacchie/Dropbox/Packages/Forestree/Vignette/Images/PlanPlacettes.png}
...
\begin{figure}[H]
\begin{center}
 \includegraphics[width=6cm]{/Users/maxbruciamacchie/Dropbox/Packages/Forestree/Vignette/Images/GB.png}
...
\begin{figure}[H]
\begin{center}
 \includegraphics[width=17cm]{/Users/maxbruciamacchie/Dropbox/Packages/Forestree/Vignette/Images/Valeurs.pdf}
